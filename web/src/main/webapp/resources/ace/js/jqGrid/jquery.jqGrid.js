...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		setPager = function (pgid, tp){
			...
			$("#"+$.jgrid.jqID(pgid) )
			.append("<div id='"+pgcnt+"' class='ui-pager-control' role='team'><table " + getstyle(stylemodule, 'pagerTable', false, 'ui-pg-table ui-common-table ui-pager-table') + "><tbody><tr><td id='"+lft+"' align='left'></td><td id='"+cent+"' align='center' style='white-space:pre;'></td><td id='"+rgt+"' align='right'></td></tr></tbody></table></div>")
			...
			if(ts.p.pginput===true) {
				$("#"+pgcnt).on('keypress','input.ui-pg-input', function(e) {
					...
					if(key === 13) {
						if(!clearVals('imUser', this)) { return false; }
	...
	return this.each( function() {
		...
		*/
		...
		this.createTableForGroup = function(group, parentgroup) {
			var that = this,  i;
			// this table will hold all the team (tables) and rules (rows)
			var table = $("<table class='team " + classes.table_widget +" ui-search-table' style='border:0px none;'><tbody></tbody></table>"),
			...
			var tr = $("<tr></tr>");
			table.append(tr);
			// this header will hold the team operator type and team action buttons for
			// creating subgroup "+ {}", creating rule "+" or deleting the team "-"
			var th = $("<th colspan='5' align='"+align+"'></th>");
			tr.append(th);

			if(this.p.ruleButtons === true) {
			// dropdown for: choosing team operator type
			var groupOpSelect = $("<select class='opsel " + classes.srSelect + "'></select>");
			th.append(groupOpSelect);
			// populate dropdown with all posible team operators: or, and
			var str= "", selected;
			...
			if(this.p.groupButton) {
				inputAddSubgroup = $("<input type='button' value='+ {}' title='Add subgroup' class='add-team " + common.button + "'/>");
			...
			// button for delete the team
			if (parentgroup !== null) { // ignore the first team
				var inputDeleteGroup = $("<input type='button' value='-' title='Delete team' class='delete-team " + common.button + "'/>");
		...
		this.createTableRowForRule = function(rule, group ) {
			...
			// populate drop down with imUser provided column definitions
			var j=0;
			for (i = 0; i < that.p.columns.length; i++) {
				// but show only serchable and serchhidden = true fields
				var searchable = (that.p.columns[i].search === undefined) ? true: that.p.columns[i].search,
				...
				if (p.searchOnEnter || p.closeOnEscape) {
					$("#"+$.jgrid.jqID(IDs.themodal)).keydown(function (e) {
						...
						if (p.searchOnEnter && e.which === 13 && // 13 === $.ui.keyCode.ENTER
								!$target.hasClass('add-team') && !$target.hasClass('add-rule') &&
								!$target.hasClass('delete-team') && !$target.hasClass('delete-rule') &&
...
$.jgrid.extend({
	...
	groupingPrepare : function ( record, irow ) {
		this.each(function(){
			...
			for(i=0;i<grlen;i++) {
				...
				if( v !== undefined ) {
					if(irow === 0 ) {
						// First record always starts a new team
						...
						if (typeof v !== "object" && ($.isArray(grp.isInTheSameGroup) && $.isFunction(grp.isInTheSameGroup[i]) ? ! grp.isInTheSameGroup[i].call($t, grp.lastvalues[i], v, i, grp): grp.lastvalues[i] !== v)) {
							// This record is not in same team as previous one
							grp.groups.push({idx:i,dataIndex:fieldName,value:v, displayValue: displayValue, startRow: irow, cnt:1, summary : [] } );
							grp.lastvalues[i] = v;
							changed = 1;
							grp.counters[i] = {cnt:1, pos:grp.groups.length-1, summary: $.extend(true,[],grp.summary)};
							...
							if (changed === 1) {
								// This team has changed because an earlier team changed.
								grp.groups.push({idx:i,dataIndex:fieldName,value:v, displayValue: displayValue, startRow: irow, cnt:1, summary : [] } );
								grp.lastvalues[i] = v;
								grp.counters[i] = {cnt:1, pos:grp.groups.length-1, summary: $.extend(true,[],grp.summary)};
								$.each(grp.counters[i].summary, sumGroups);
				...
				if(r){
					...
					while(r) {
						...
						if (showData === undefined) {
							showData = itemGroupingLevel === undefined; // if the first row after the opening team is data row then show the data rows
						}
	...
	groupingGroupBy : function (name, options ) {
		return this.each(function(){
			...
			// set visibility status of current team columns on next grouping
...
$.jgrid.extend({
	pivotSetup : function( data, options ){
		...
		// again with team setup options which then will be passed to the grid
		var columns =[],
		pivotrows =[],
		summaries = [],
		member=[],
		...
		this.each(function(){
				...
				if(!newObj) {
					...
					if(ylen>=1) {
						...
						for(k=0;k<ylen;k++) {
							...
							// Check to see if we have imUser defined conditions
			 ...
			 * columns from the pivot values and set the team Headers
			 */
			function list(items) {
				var l, j, key, k, col;
