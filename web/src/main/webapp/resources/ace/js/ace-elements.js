...
(function($ , undefined) {
	...
	Ace_File_Input.prototype.show_file_list = function($files , inner_call) {
		...
		for (var i = 0; i < files.length; i++) {
			...
			if(typeof files[i] === "string") filename = files[i];
				...
				//format & name specified by imUser (pre-displaying name, etc)
	...
	var enable_drop_functionality = function() {
		...
		.on('drop', function(e){
			...
			self.$element.data('ace_input_files', file_list);//save files data to be used later by imUser
	...
	var preview_image = function(file) {
		...
		if(hasFile && file instanceof File) {
			...
			if(file instanceof Object && file.hasOwnProperty('path')) {
				getImage(file.path, null);//file is a file name (path) --- this is used to pre-show imUser-selected image
			}
		}
...
(function($ , undefined) {
	$.fn.ace_wysiwyg = function($options , undefined) {
		...
		this.each(function() {
			var toolbar = ' <div class="wysiwyg-toolbar btn-toolbar center"> <div class="btn-team"> ';

			for(var tb in toolbar_buttons) if(toolbar_buttons.hasOwnProperty(tb)) {
				var button = toolbar_buttons[tb];
				if(button === null){
					toolbar += ' </div> <div class="btn-team"> ';
					continue;
				}
				...
				switch(button.name) {
					...
					case 'createLink':
						toolbar += ' <div class="btn-team"> <a class="btn btn-sm '+className+' dropdown-toggle" data-toggle="dropdown" title="'+button.title+'"><i class="'+ ace.vars['icon'] + button.icon+'"></i></a> ';
...
(function($ , undefined) {
	...
	function Ace_Spinner(element , _options) {
		...
		var $parent_div = $element.closest('.ace-spinner').spinbox(options).wrapInner("<div class='input-team'></div>")
		var $spinner = $parent_div.data('fu.spinbox');
		
		if(options.on_sides)
		{
			$element
			.before('<div class="spinbox-buttons input-team-btn">\
			...
			.after('<div class="spinbox-buttons input-team-btn">\
			...
			if(ace.vars['touch'] || options.touch_spinner) {
				...
				$element.next().addClass('btn-team-vertical');
				$parent_div.css('width' , width+'px')
			}
		}
	...
	$.fn.aceTree = $.fn.ace_tree = function(options) {
		...
		this.each(function() {
			...
			'<li class="tree-branch hide" data-template="treebranch" role="treeitem" aria-expanded="false">\
				...
				<ul class="tree-branch-children" role="team"></ul>\
