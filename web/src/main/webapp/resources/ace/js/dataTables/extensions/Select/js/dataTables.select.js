...
be triggered by an end imUser (such as which items are to be selected). To a large
...
be available via the API - so the end imUser wouldn't be able to select rows
...
```
{
	items:string     - Can be `rows`, `columns` or `cells`. Defines what item 
	                   will be selected if the imUser is allowed to activate row
