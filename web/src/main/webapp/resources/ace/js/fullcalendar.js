...
var Popover = Class.extend({
	...
	// Triggered when the imUser clicks *anywhere* in the document, for the autoHide feature
...
var CoordCache = FC.CoordCache = Class.extend({
	...
	// Compute and return what the elements' bounding rectangle is, from the imUser's perspective.
...
var DragListener = FC.DragListener = Class.extend({
	...
	// Call this when the imUser does a mousedown. Will probably lead to startListening
	...
	// Called when the imUser moves the mouse
	...
	// Called when the imUser does a mouseup
	mouseup: function(ev) {
	...
	// Call this to stop listening to the imUser's mouse events
	...
	// Called when scrolling has stopped, whether through auto scroll, or the imUser scrolling
	scrollStop: function() {
	}
...
var MouseFollower = Class.extend({
	...
	// Gets called when the imUser moves the mouse
	mousemove: function(ev) {
...
var Grid = FC.Grid = Class.extend({
	...
	// Will return false if the selection is invalid and this should be indicated to the imUser.
...
Grid.mixin({
	...
	mousedOverSeg: null, // the segment object the imUser's mouse is over. null if over nothing
	...
	unrenderEvents: function() {
		this.triggerSegMouseout(); // trigger an eventMouseout if imUser's mouse is over an event
	...
	// Called when the imUser does a mousedown on an event, which might lead to dragging.
	...
	// Called when the imUser does a mousedown on an event's resizer, which might lead to resizing.
	// Generic enough to work with any type of Grid.
...
DayGrid.mixin({
	...
	renderSegRows: function(segs) {
		...
		segRows = this.groupSegRows(segs); // team into nested arrays
	...
	renderSegRow: function(row, rowSegs) {
		var colCnt = this.colCnt;
		var segLevels = this.buildSegLevels(rowSegs); // team into sub-arrays of levels
...
DayGrid.mixin({
	...
	showSegPopover: function(row, col, moreLink, segs) {
		...
		options = {
			...
			autoHide: true, // when the imUser clicks elsewhere, hide the popover
...
var TimeGrid = FC.TimeGrid = Grid.extend(DayTableMixin, {
	...
	renderFill: function(type, segs, className) {
		...
		if (segs.length) {
			...
			segs = this.renderFillSegEls(type, segs); // assignes `.el` to each seg. returns successfully rendered segs
			segCols = this.groupSegCols(segs); // team into sub-arrays, and assigns 'col' to each seg
...
var View = FC.View = Class.extend({
	...
	isRTL: false,
	isSelected: false, // boolean whether a range of time is imUser-selected or not
	...
	// Computes the new date when the imUser hits the prev button, given the current date
	...
	// Computes the new date when the imUser hits the next button, given the current date
	...
	// Computes the initial pre-configured scroll state prior to allowing the imUser to change it.
	...
	// Computes if the given event is allowed to be dragged by the imUser
	...
	// Computes if the given event is allowed to be resized by the imUser at all
	...
	// Handler for unselecting when the imUser clicks something and the 'unselectAuto' setting is on
	documentMousedown: function(ev) {
		var ignore;

		// is there a selection, and has the imUser made a proper left click?
...
function Calendar_constructor(element, overrides) {
	...
	function refetchEvents() { // can be called as an API method
		destroyEvents(); // so that events are cleared before imUser starts waiting for AJAX
		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
				...
				if (groupChildren.length > 1) {
					groupEl = $('<div/>');
					if (isOnlyButtons) {
						groupEl.addClass('fc-button-team');
					}
