...
(function ($) {
       ...
       activate: function() {
			...
			if(!this.nativeUI) {
				...
				if(picker) {
					...
					.siblings('.input-team-addon').on('click', function(){
						picker.show();
					})
				}
			}
    ...
    ADate.defaults = $.extend({}, $.fn.editabletypes.abstractinput.defaults, {
        tpl:'<div class="input-team input-team-compact"><input type="text" class="input-medium date" /><span class="input-team-addon"><i class="fa fa-calendar"></i></span></div>',
		date: {
			weekStart: 0,
            startView: 0,
            minViewMode: 0
