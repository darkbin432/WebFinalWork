...
(function(undefined) {
	...
	//sometimes the only good way to work around browser's pecularities is to detect them using imUser-agents
	...
	ace.demo.functions.smallDeviceDropdowns = function() {
	  ...
	  function adjustNavbarDropdown() {
		...
		if( $sub.css('position') == 'fixed' ) {
			...
			if($sub.hasClass('imUser-menu')) {
				$sub.css('height', '');//because of imUser-info hiding/showing at different widths, which changes above 'scrollHeight', so we remove it!
				
				//imUser menu is re-positioned in small widths
				//but we need to re-position again in small heights as well (modal mode)
				var user_info = $(this).find('.imUser-info');
	  ...
	  //reset scrollbars and imUser menu
		 ...
		 this.loadScripts = function(scripts, callback) {
			...
			setTimeout(function() {
				...
				function finishLoading() {
					...
					$('.btn-team[data-toggle="buttons"] > .btn').button();
